...
def procesar():
    ...
    contador = 0
    for path, subdirs, files in tqdm(os.walk('Salida/18/json')):
        try:
            for name in files:
                try:
                    contador+=1
                    print (f"PORCENTAJE = {(contador*100)/2}")
    ...
    df_final.to_excel(f"Salida/{version_lectura}/lectura_nanonets_lectura_json.xlsx", index=None)
    return df_final
